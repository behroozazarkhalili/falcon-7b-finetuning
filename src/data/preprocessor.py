...
class DataPreprocessor:
    ...
    def preprocess_dataset(self, dataset: Dataset, batch_size: int = 1000) -> Dataset:
        ...
        self.logger.info(f"Dataset preprocessing completed. New columns: {processed_dataset.column_names}")
    ...
    def format_instruction_dataset(self, dataset: Dataset) -> Dataset:
        ...
        formatted_dataset = dataset.map(
            format_function,
            batched=True,
            desc="Formatting for instruction tuning"
        )
    ...
    def split_dataset(self, dataset: Dataset, train_ratio: float = 0.9) -> Dict[str, Dataset]:
        ...
        self.logger.info(f"Dataset split - Train: {len(train_dataset)}, Eval: {len(eval_dataset)}")
    ...
    def validate_tokenization(self, dataset: Dataset) -> bool:
        ...
        for col in required_columns:
            if col not in dataset.column_names:
                raise ValueError(f"Required column '{col}' not found after tokenization")
        
        # Check token lengths
        input_ids = dataset["input_ids"]
        for i, ids in enumerate(input_ids[:10]):  # Check first 10 samples
            if len(ids) > self.max_length:
                raise ValueError(f"Sample {i} exceeds max_length: {len(ids)} > {self.max_length}")
