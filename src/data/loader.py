...
class DataLoader:
    ...
    def load_dataset(self) -> Dataset:
        ...
        try:
            ...
            if self.dataset_config.max_samples:
                dataset = dataset.select(range(min(self.dataset_config.max_samples, len(dataset))))
    ...
    def _validate_dataset(self, dataset: Dataset) -> None:
        ...
        if self.dataset_config.text_field not in dataset.column_names:
            raise ValueError(f"Text field '{self.dataset_config.text_field}' not found in dataset")
    ...
    def _apply_quality_checks(self, dataset: Dataset) -> Dataset:
        ...
        if quality_config.get("check_empty_text", True):
            dataset = dataset.filter(
                lambda x: x[self.dataset_config.text_field] and 
                         len(x[self.dataset_config.text_field].strip()) > 0
            )
            self.logger.info(f"Filtered empty text: {original_size} -> {len(dataset)}")
        
        # Check text length
        min_length = quality_config.get("min_text_length", self.dataset_config.min_length)
        max_length = quality_config.get("max_text_length", 2048)
        
        dataset = dataset.filter(
            lambda x: min_length <= len(x[self.dataset_config.text_field]) <= max_length
        )
        self.logger.info(f"Filtered by length ({min_length}-{max_length}): {len(dataset)} samples")
