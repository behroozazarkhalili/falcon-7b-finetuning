...
class FalconTrainer:
    ...
    def __init__(
        self, model: PreTrainedModel, tokenizer: PreTrainedTokenizer, config: DictConfig
    ):
        """
    ...
    def _init_wandb(self) -> None:
        ...
        try:
            ...
            if self.wandb_manager:
                ...
                logger.info(
                    f"Wandb initialized. Run URL: {self.wandb_manager.get_run_url()}"
                )
    ...
    def train(
        ...
        if self.wandb_manager:
            self.wandb_manager.log_metrics(
                {"final_" + k: v for k, v in metrics.items()}
            )
    ...
    def evaluate(self, eval_dataset: Optional[Dataset] = None) -> Dict:
        ...
        if self.wandb_manager:
            self.wandb_manager.log_metrics(
                {"eval_" + k: v for k, v in eval_result.items()}
            )
