...
class WandbManager:
    ...
    def init_wandb(self, resume: Optional[str] = None) -> Any:
        ...
        init_args = {
            ...
            or self.experiment_config.get("description"),
    ...
    def watch_model(self, model: PreTrainedModel) -> None:
        ...
        if settings.get("watch_model", True):
            ...
            if log_type:
                ...
                self.logger.info(
                    f"Watching model with log_type: {log_type}, freq: {log_freq}"
                )
    ...
    def log_model_checkpoint(
        self, checkpoint_path: Union[str, Path], step: int
    ) -> None:
        """
...
def init_wandb_from_config(
    config: DictConfig, resume: Optional[str] = None
) -> Optional[WandbManager]:
    """
